		...
		select!{
			_ = async {
				state_data.edit_len +=	input
					.read_edit(&mut state_data.buffer[<i16 as TryInto<usize>>::try_into(state_data.edit_len).unwrap()..])
					.await
					.unwrap();
				state_data.edit_len = if state_data.edit_len < 0 { 0 } else { state_data.edit_len };
				state_data.edit_len = if state_data.edit_len > 128 { 128 } else { state_data.edit_len };
			}.fuse() => {
